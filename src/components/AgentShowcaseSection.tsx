...
const agents = [
  ...
  },
  {
    ...
    features: [
      ...
      "Responds to DMs and comments 24/7 — 45% of leads come in after hours",
      "Instantly engages prospects so no opportunity goes cold",
      "Drives 21× more sales and booked consultations",
      "Converts at 45%, beating traditional outreach methods",
    ],
    hasCalculator: false,
    ...
    <section id="popular-agents" className="section-light py-16 md:py-24 border-t" style={{ borderColor: "hsl(var(--light-border))" }}>
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center mb-10 md:mb-16">
          <div
            className="flex md:inline-flex md:flex-wrap justify-start md:justify-center gap-1 rounded-2xl md:rounded-full p-1.5 overflow-x-auto max-w-full scrollbar-hide"
          ...
          >
            {agents.map((a) => (
              <button
                ...
                className={`flex items-center gap-1.5 md:gap-2 rounded-full px-3 md:px-5 py-2 md:py-2.5 text-xs md:text-sm font-medium transition-all duration-200 whitespace-nowrap shrink-0 ${
                  activeId === a.id
              ...
              >
                <a.icon size={14} className="md:w-4 md:h-4" />
                <span>{a.label}</span>
              </button>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {active.stats && (
            <div className="mb-8 md:mb-12">
              ...
              {active.statsCta && (
                ...
                >
                  <p className="text-xs md:text-sm font-semibold" style={{ color: "hsl(var(--primary))" }}>
                    {active.statsCta}
                  </p>
                </div>
              )}
          ...
          <div className="grid lg:grid-cols-5 gap-8 md:gap-12 items-start">
            ...
            <div className="lg:col-span-3">
              <h3 className="text-xl md:text-3xl font-bold mb-3 md:mb-4" style={{ color: "hsl(var(--light-fg))" }}>
                {active.title}
              </h3>
              <p className="text-xs md:text-sm leading-relaxed mb-6 md:mb-8" style={{ color: "hsl(var(--light-muted))" }}>
                {active.description}
              </p>

              <div className="space-y-2.5 md:space-y-3 mb-6 md:mb-8">
                {active.features.map((f) => (
                  <div key={f} className="flex items-start gap-2.5 md:gap-3">
                    <CheckCircle2
                      size={16}
                      className="shrink-0 mt-0.5 md:w-[18px] md:h-[18px]"
                      style={{ color: "hsl(var(--primary))" }}
                    />
                    <span className="text-xs md:text-sm" style={{ color: "hsl(var(--light-fg))" }}>
                      {f}
                    </span>
                  </div>
                ))}
              ...
              <Button size="lg" className="text-xs md:text-sm font-semibold gap-2 w-full sm:w-auto" asChild>
                <a href="#contact">
            ...
            <div className="lg:col-span-2">
              <div
                className="rounded-2xl border p-4 md:p-6 shadow-lg"
                style={{
                  borderColor: "hsl(var(--light-border))",
                  background: "hsl(var(--light-card))",
                }}
              >
                <div className="text-xs md:text-sm font-semibold mb-3 md:mb-4" style={{ color: "hsl(var(--light-fg))" }}>
