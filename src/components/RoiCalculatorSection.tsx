  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="relative h-[18px] flex items-center">
        ...
        <div className="absolute h-[4px] rounded-full" style={{ width: `${pct}%`, backgroundColor: "#0f172a" }} />
        <input
          type="range" min={min} max={max} step={step} value={value}
          onChange={e => onChange(Number(e.target.value))}
          className="roi-range-embed absolute w-full"
        />
      </div>
...
const ResultRow = ({
  ...
  <div className={`py-5 ${border ? "border-b" : ""}`} style={{ borderColor: "#f1f5f9" }}>
    <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color }}>{label}</p>
    <div className="flex items-baseline gap-2">
      <span className="text-3xl font-black tracking-tight" style={{ color: "#0f172a" }}>{monthly}</span>
      <span className="text-sm font-medium" style={{ color: "#9ca3af" }}>/mo</span>
    </div>
    <p className="text-sm font-semibold mt-0.5" style={{ color }}>{annual} /year</p>
  </div>
);
  ...
  return (
    <section className="py-20 px-6 bg-white">
      <style>{`
        ...
        .roi-range-embed::-webkit-slider-thumb {
          -webkit-appearance: none;
        ...
        .roi-range-embed::-webkit-slider-thumb:hover {
          transform: scale(1.15);
          box-shadow: 0 2px 8px rgba(0,0,0,0.22);
        }
        .roi-range-embed::-moz-range-thumb {
          width: 20px; height: 20px;
          border-radius: 50%;
          background: #0f172a;
          border: 3px solid #ffffff;
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-sm" style={{ color: "#6b7280" }}>
            Adjust the sliders to get your real-time revenue recovery estimate.
          </p>
        </div>
        ...
        >
          ...
          >
            <div>
              <h3 className="text-sm font-bold" style={{ color: "#0f172a" }}>Missed Call Savings Calculator</h3>
              <p className="text-xs mt-0.5" style={{ color: "#9ca3af" }}>Enter your numbers to see real-time revenue estimates.</p>
            </div>
            <span
          ...
          <div className="flex flex-col lg:flex-row bg-white">
            ...
            {/* Left: sliders */}
            <div
              className="flex-1 px-8 py-7 flex flex-col gap-6 border-b lg:border-b-0 lg:border-r"
            ...
            >
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-semibold w-fit transition-opacity hover:opacity-70"
                style={{ color: "#0ea5e9" }}
              >
              ...
              {showAdv && (
                <div className="flex flex-col gap-6">
                  <p className="text-xs -mt-2" style={{ color: "#9ca3af" }}>These defaults are adjustable.</p>
                  <Slider label="Close rate" value={closeRate} min={5} max={80} step={1} display={`${closeRate}%`} onChange={setCloseRate} />
                  <Slider label="AI recovery rate" value={aiRecovery} min={50} max={100} step={1} display={`${aiRecovery}%`} onChange={setAiRecovery} />
                  <Slider label="Gross margin" value={grossMargin} min={10} max={90} step={1} display={`${grossMargin}%`} onChange={setGrossMargin} />
                </div>
              )}
            </div>

            {/* Right: results */}
            <div className="lg:w-80 xl:w-96 px-8 py-7 flex flex-col">
              <div className="flex-1">
                <ResultRow label="Estimated Revenue Missed" monthly={fmtK(revMissedMo)} annual={fmtK(revMissedMo * 12)} color="#dc2626" />
                <ResultRow label="Revenue Recovered with StratixOS" monthly={fmtK(recoveredMo)} annual={fmtK(recoveredMo * 12)} color="#0d9488" />
                <ResultRow label="Profit Recovered" monthly={fmtK(profitMo)} annual={fmtK(profitMo * 12)} color="#0d9488" border={false} />
              </div>

              <div className="flex flex-col gap-3 mt-6 pt-5 border-t" style={{ borderColor: "#f1f5f9" }}>
                <Link
                  to="/automations"
